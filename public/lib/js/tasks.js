function parseData(tasks){
	...
	for (i in tasks){
		...
		code += "<div class='task card'" +
				"data-id='" + i + "' " +
				"data-type='" + task.type + "' " +
				"data-date='" + (new Date(task.stats.lastRun)).getTime() + "' " + 
				"data-count='" + task.stats.total + "' >";
...
function style(str){
	if (str.length < 9) return "style='font-size: 3em;'";
	//else if (str.length < 15) return "style='font-size: 2.2em;'";
